...
class TestDatabase(unittest.TestCase):
    def setUp(self):
        ...
        def test_crear_clientes(self):
            nuevo_cliente = db.Clientes.crear('39X', 'Josias', 'Aponte')
            self.assertEqual(len(db.Clientes.lista), 4)
            self.assertEqual(nuevo_cliente.dni, '39X')
            self.assertEqual(nuevo_cliente.nombre, 'Josias')
            self.assertEqual(nuevo_cliente.apellido, 'Aponte')
        ...
        def test_dni_valido(self):
            ...
            self.assertFalse(helpers.dni_valido('1231231S', db.Clientes.lista))
